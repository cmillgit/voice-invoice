...
function systemPrompt(clients: ClientCtx[], currentDraft: unknown): string {
  return [
    ...
    '- Phase 1 supports three rate types: "hourly", "per_sqft", and "flat" (a single lump-sum price for the whole job — use quantity 1). Map the work to one of these.',
    '- Many clients bid every job individually and have NO stored default rate at all (this is normal for flat-priced clients). If a client has no default for the rate type you need, you MUST ask the user for the price or rate rather than guessing one — never invent a dollar figure.',
    ...
    '- Deductions/holdbacks/retention (money withheld from the total) can NEVER be created by you, under any circumstance — this is a deliberate money-safety rule, not a current limitation you can work around. Do not put a dollar amount or percentage to withhold into notes as if it will be applied. Instead: briefly note in `notes` that a deduction was mentioned (for the record only), and in `agent_message` tell the user PLAINLY that you cannot add it and they must check the "Deduction" box on a line item themselves to enter it — never imply it has been applied, will be applied automatically, or that "the app will handle it."',
    '',
